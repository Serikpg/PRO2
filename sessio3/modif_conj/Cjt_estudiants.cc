...
/* Post: b indica si el parametre implicit original conte un estudiant amb el
   dni d’est; si b=fals, s’ha afegit l’estudiant est al parametre implicit */
{
...
int Cjt_estudiants::cerca_dicot_new(const vector<Estudiant>& vest, int left, int right, 
									int x, bool& b) {
/* Pre: vest esta ordenat */
  int i;
  b = false;
...
{
  if (nest >= MAX_NEST) throw PRO2Excepcio("Conjunt ple");
  int i = cerca_dicot_new(vest, 0, nest-1, est, b);
  vest[i+1] = est;
  ++nest;
}
...
void Cjt_estudiants::modificar_estudiant(const Estudiant& est, bool& b) {
  ...
  if (i == -1) b = false; // si el PI original no conte un estudiant --> b = false
  else { // si el PI original conte un estudiant --> modifica'l i b = true
	  b = true;
	  vest[i] = est;
  }
}    
  		
void Cjt_estudiants::modificar_iessim(int i, const Estudiant& est, bool& b) {
...
void Cjt_estudiants::consultar_estudiant(Estudiant& est, int dni, bool& b) {
	int i = cerca_dicot(vest, 0, nest-1, dni);
	if (i == -1) b = false; // si l'estudiant consultat no existeix al PI --> b = false
	else { // si l'estudiant consultat existeix al PI
		b = true;
		est = vest[i];
	}
}
