...
void Cjt_estudiants::recalcular_posicio_imax()
{
  for (int i = 0; i < nest; ++i) 
    if (vest[imax].te_nota() and vest[i].te_nota() and
		vest[imax].consultar_nota() < vest[i].consultar_nota()) 
		imax = i; 
}
...
void Cjt_estudiants::afegir_estudiant(const Estudiant &est)
{
  ...
  if (not vest[imax].te_nota() and est.te_nota()) imax = i+1;
  else if ((est.te_nota() and 
		   vest[imax].consultar_nota() < est.consultar_nota()) 
		  or
		   (est.te_nota() and
		   vest[imax].consultar_nota() == est.consultar_nota() and
		   vest[imax].consultar_DNI() > est.consultar_DNI() ))
	  imax = i+1;
}
